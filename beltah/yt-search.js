...
zokou({ nomCom: "yts", categorie: "Search", reaction: "✋" }, async (dest, zk, commandeOptions) => {
  ...
  if (!query[0]) {
    repondre("idiot kot sa wal fh rechech la ?");
    return;
  }
...
zokou({
  ...
  if (!arg[0]) {
    repondre("mete lien youtube la baz");
    return;
  }
...
_*En cours de téléchargement...*_\n\n`
    ...
    */
    ...
    fileStream.on('finish', () => {
      ...
      zk.sendMessage(origineMessage, { video: { url: `./${filename}` }, caption: "Powered by *ANDY MRLIT*", gifPlayback: false }, { quoted: ms });
...
zokou({
  ...
  if (!arg[0]) {
    repondre("mete lien video ytb la");
    return;
  }
