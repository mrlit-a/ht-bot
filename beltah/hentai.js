...
async (origineMessage, zk, commandeOptions) => {
  const { repondre, ms ,verifGroupe , superUser} = commandeOptions;
   ...
   if (!verifGroupe && !superUser ) { repondre(`bro relax non komand sa se pou group selman.`) ; return ;}
   
    let isHentaiGroupe = await hdb.checkFromHentaiList(origineMessage) ;

    if(!isHentaiGroupe && !superUser) { repondre(`Gwoup sa pa yon gwoup perverti, kalme w zanmim.`) ; return ;}
...
async (origineMessage, zk, commandeOptions) => {
  ...
  const { repondre, ms ,verifGroupe , superUser} = commandeOptions;
   ...
   if (!verifGroupe && !superUser ) { repondre(`baz koamnd sa se nan group selman ou ka fel lanmed.`) ; return ;}
